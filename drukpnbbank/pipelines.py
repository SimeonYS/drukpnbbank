...
class DdrukpnbbankPipeline:

    # Database setup
    conn = sqlite3.connect('drukpnbbank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `drukpnbbank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM drukpnbbank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO drukpnbbank (date, title, link, content)"
